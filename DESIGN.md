...
title: "Owen Browser Bridge Design"
status: adopted
product: "owen-extension"
surface: "VS Code capture explorer and browser extension popup"
kind: plugin
updated: 2026-08-12
...
# Owen Browser Bridge Design

## Decision Summary

| 항목 | 결정 |
| --- | --- |
| 사용자 | Browser portal evidence를 Copilot과 함께 조사하는 보안 분석가와 개발자 |
| Output job | 현재 tab을 안전하게 capture하고 검증 가능한 evidence로 검색·검토·agent에 전달한다 |
| Primary object | browser capture, screenshot, DOM metadata와 action request |
| Primary action | Capture를 생성하거나 preview된 browser action을 승인한다 |
| Composition | VS Code workbench panel + compact browser popup + evidence review gate |
| 핵심 판단 | Capture·redaction·human approval의 신뢰 경계를 편의 기능보다 우선한다 |

## Context

Owen Browser Bridge는 VS Code LM tools와 Manifest V3 browser extension을 연결한다. Popup은 pairing과 capture를, VS Code panel은 evidence 탐색을, agent action UI는 preview와 승인 경계를 담당한다.
...
- README.md
- AGENTS.md
- capture-integrity.ts
- browser-actions.ts
- agent-action-registry.ts

## Foundation Selection

### Adopted

- Popup은 340px compact settings/workflow
- Capture list와 extracted element body는 opaque
- Pairing·capture quality·redaction 상태를 text/icon으로 표시
- High-risk browser action은 preview와 human review gate 필수

### Excluded Or App-Owned

- Password·token·secret capture
- 사용자 승인 없는 click·type·submit
- Screenshot을 성공 evidence로 단독 간주

## Design Decisions

### D-01. Pairing state를 첫 화면에서 확인한다

- **결정**: paired, connecting, unpaired, error를 popup header와 recovery action으로 표시한다
- **이유**: Capture 실패 원인을 portal 문제로 오해하지 않게 한다
- **검증**: Bridge offline, wrong port, expired pairing fixture를 검증한다

### D-02. Capture quality를 evidence 가까이에 둔다

- **결정**: URL, timestamp, load/auth state, redaction과 quality score를 capture row에 표시한다
- **이유**: 불완전 capture를 사실 근거로 사용하지 않게 한다
- **검증**: Partial load와 auth screen sample의 score·warning을 검증한다

### D-03. Browser action은 preview 후 실행한다

- **결정**: Target highlight, action description, risk와 expected effect를 보여주고 명시적으로 승인한다
- **이유**: Agent automation이 사용자 의도와 portal state를 넘어가지 않게 한다
- **검증**: Cancel, stale target, no-effect, destructive action guard를 E2E로 확인한다
...
- npm typecheck, unit and extension tests
- Browser popup 340px keyboard and state audit
- Capture redaction and browser action guard tests
